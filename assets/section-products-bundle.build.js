	...
	var ProductsBundle = () => {
		...
		function changeVariantHandler({ event, section, sectionId, buttonLabels }) {
			...
			buttons.forEach((button) => {
				const buttonText = button.querySelector(selectors.buttonText) || button;

				button.disabled = !enableButton;

				if (enableButton) {
					buttonText.innerHTML = buttonLabels.enabled;
				} else {
					buttonText.innerHTML = buttonLabels.disabled;
				}
			});
		...
		function updateTotal(section) {
			...
			if (amount !== null) {
				totalPrices.forEach((totalPrice) => {
					totalPrice.innerHTML = formatMoney(amount);
				});
			}
		}
